...
import github.businessdirt.axite.vanadium.core.utils.VulkanUtils.PORTABILITY_EXTENSION
...
class Instance(config: VanadiumConfig) : Handle<VkInstance>() {
    ...
    override val handle: VkInstance = memoryStack { stack ->
        ...
        val usePortability = PORTABILITY_EXTENSION in VulkanUtils.instanceExtensions && PlatformUtils.type == Platform.MACOS
        ...
        val instanceInfo = VkInstanceCreateInfo.calloc(stack).`sType$Default`()
            .pApplicationInfo(appInfo)
            .ppEnabledLayerNames(requiredLayers)
            .ppEnabledExtensionNames(requiredExtensions)

        if (usePortability) instanceInfo.flags(0x00000001) // VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR
    ...
    override fun destroy() = vkDestroyInstance(handle, null)

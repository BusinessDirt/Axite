...
class Image(
    ...
    override val handle: Long = memoryStack { stack ->
        ...
        stack.createHandle({ "Failed to create image" }) {
            vkCreateImage(Context.device.handle, imageCreateInfo, null, it)
        }
    }
    ...
    val memoryHandle: Long = memoryStack { stack ->
        ...
        vkGetImageMemoryRequirements(Context.device.handle, handle, memoryRequirements)

        // Select memory size and type
        val memoryAllocationInfo = VkMemoryAllocateInfo.calloc(stack).`sType$Default`()
                .allocationSize(memoryRequirements.size())
                .findMemoryTypeIndex(memoryRequirements.memoryTypeBits(), VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)
        ...
        stack.createHandle({ "Failed to allocate memory" }) {
            ...
            vkCheck(vkBindImageMemory(Context.device.handle, handle, it, 0)) {

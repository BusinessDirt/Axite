...
void main() {
    vec4 color;
    if (USE_FXAA == 1) {
        color = fxaa(inputTexture, inUV);
    } else {
        color = texture(inputTexture, inUV);
    }

    if (EFFECT_TYPE == 1) { // Grayscale
        float gray = dot(color.rgb, vec3(0.299, 0.587, 0.114));
        color = vec4(vec3(gray), color.a);
    } else if (EFFECT_TYPE == 2) { // Invert
        color = vec4(1.0 - color.rgb, color.a);
    } else if (EFFECT_TYPE == 3) { // Pixelate
        vec2 pixelSize = 10.0 / screenSize.size;
        vec2 uv = floor(inUV / pixelSize) * pixelSize;
